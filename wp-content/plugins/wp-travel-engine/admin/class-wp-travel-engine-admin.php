...
class Wp_Travel_Engine_Admin
{
	...
	public function enqueue_scripts()
	{
		...
		if ($screen->post_type == 'trip' || $screen->post_type == 'booking' || isset($_GET['page']) && $_GET['page'] == 'class-wp-travel-engine-admin.php' || $screen->id == 'trip_page_class-wp-travel-engine-admin') {
			...
			wp_enqueue_script($this->plugin_name, plugin_dir_url(__FILE__) . 'js'. $asset_script_path. 'wp-travel-engine-admin'. $version_prefix .'.js', array('jquery', 'jquery-ui-core', 'jquery-ui-tabs', 'jquery-ui-sortable'), $this->version, true);
		...
		wp_enqueue_script('select2', plugin_dir_url(__FILE__) . 'js'.$asset_script_path.'select2'. $version_prefix .'.js', array('jquery'), '5.6.3', true);
	}
